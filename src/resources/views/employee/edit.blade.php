...
@section('title', '社員変更 | ' . config('app.name'))

@section('content')
<div class="h-full ml-14 mt-20 mb-10 md:ml-64">
  <div class="flex mt-10 mx-4"> 
    <h2 class="font-medium text-gray-800 mr-5 dark:text-gray-400">社員変更</h2>
  </div>
  ...
  <div class="mx-4 mt-5">
    <div class="w-full mb-2 rounded-lg shadow-xs dark:border-gray-700 bg-gray-50 sm:grid-cols-9 dark:text-gray-400 dark:bg-gray-800">
      <div class="flex flex-wrap">
        <form method="POST" action="{{ route('employee.update', $employee->id) }}" class="w-full h-adr">
          @csrf
          ...
          <div class="md:w-4/5 mx-auto p-2">
            ...
            <div class="flex justify-between">
              <input type="text" id="last_name_kana" name="last_name_kana" value="{{ old('last_name_kana', $employee->last_name_kana) }}"
                class="w-full mr-2 bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
              <input type="text" id="first_name_kana" name="first_name_kana" value="{{ old('first_name_kana', $employee->first_name_kana) }}"
                class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
            </div>
            ...
            <div>
              <select name="contract_type_id"
                ...
                @foreach($contract_types as $key => $contract_type)
                  <option value="{{ $key }}" @if($key === (int)old('contract_type_id', $employee->contract_type_id)) selected @endif>{{ $contract_type }}</option>
                @endforeach
              </select>
            </div>
          ...
          <div class="md:w-4/5 mx-auto p-2">
            ...
            <input type="text" id="post_code" name="post_code" value="{{ old('post_code', $employee->post_code) }}" maxlength="8"
          ...
          <div class="md:w-4/5 mx-auto p-2">
            ...
            <input type="text" id="phone_number" name="phone_number" value="{{ old('phone_number', $employee->phone_number) }}" maxlength="21"
              class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
            <x-input-error :messages="$errors->get('phone_number')" class="mt-2 dark:text-gray-400" />
          </div>
          <div class="md:w-4/5 mx-auto p-2">
            <label for="email" class="leading-7 text-sm text-gray-600">メールアドレス</label>
            <input type="text" id="email" name="email" value="{{ old('email', $employee->email) }}" maxlength="254"
              class="w-full bg-gray-100 bg-opacity-50 rounded border border-gray-300 focus:border-indigo-500 focus:bg-white focus:ring-2 focus:ring-indigo-200 text-base outline-none text-gray-700 py-1 px-3 leading-8 transition-colors duration-200 ease-in-out">
          ...
          <div class="md:w-4/5 mx-auto p-2">
            ...
            <input-date name="birthday" :old-birthday="{{ Illuminate\Support\Js::from(old('birthday', $employee->birthday)) }}"></input-date>
