...
@section('title', '作業一覧 | ' . config('app.name'))
...
<div class="h-full ml-14 mt-20 mb-10 md:ml-64">
  <div class="flex mt-10 mx-4"> 
    ...
    <form method="GET"action="{{ route('task_assign.index') }}">
    ...
    <button onclick="location.href='{{ route("task_assign.create") }}'" class="flex text-xs ml-auto text-white bg-blue-800 border-0 py-2 px-6 focus:outline-none hover:bg-blue-600 dark:text-gray-400 dark:bg-gray-800 dark:hover:bg-gray-600">
    ...
    <div class="w-full overflow-hidden rounded-lg shadow-xs">
      <div class="w-full overflow-x-auto">
        <div class="table table-auto w-full">
          ...
          @foreach($task_assigns as $workplace_id => $task_assign)
            <a href="{{ route('task_assign.edit', $workplace_id) }}" class="table-row border-current divide-y bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-900 text-gray-700 dark:text-gray-400">
              <div class="table-cell px-4 py-3 text-sm border-t">{{ $task_assign[0]->workplaces->clients->client_name }}</div>
              <div class="table-cell px-4 py-3 text-sm">{{ $task_assign[0]->workplaces->name }}</div>
              <div class="table-cell px-4 py-3 text-sm">
                @foreach($task_assign as $ta)
                  <div>{{ $ta->employees->fullname }}</div>
                @endforeach</div>
            </a>
          @endforeach
        </div>
      </div>
      {{ $task_assigns->links('vendor.pagination.custom') }}
    </div>
  </div>
</div>
@endsection