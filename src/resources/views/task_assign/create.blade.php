...
@section('title', '作業配分登録 | ' . config('app.name'))

@section('content')
<div class="h-full ml-14 mt-20 mb-10 md:ml-64">
  <div class="flex mt-10 mx-4"> 
    <h2 class="font-medium text-gray-800 mr-5 dark:text-gray-400">作業配分登録</h2>
  </div>
  <div class="mx-4 mt-5">
    <div class="w-full mb-2 rounded-lg shadow-xs dark:border-gray-700 bg-gray-50 sm:grid-cols-9 dark:text-gray-400 dark:bg-gray-800">
      <div class="flex flex-wrap">
        <form method="POST" action="{{ route('task_assign.store') }}" class="w-full">
          @csrf
              ...
              <select name="workplace_id"
                ...
                @foreach($clients as $client)
                  @foreach($client->workplaces as $workplace)
                    <option value="{{ $workplace->id }}" @if($workplace->id === (int)old('workplace_id')) selected @endif>{{ $workplace->name }}</option>
                  @endforeach
                @endforeach
              </select>
            </div>
          ...
          <div class="md:w-4/5 mx-auto p-2">
            ...
            <employee :employees={{ Illuminate\Support\Js::from($employees) }} :old-employee-ids={{ Illuminate\Support\Js::from(old('employee_ids')) }}></employee>
