      ...
      <div class="mx-4 mt-5">
        <div class="w-full mb-2 rounded-lg shadow-xs dark:border-gray-700 bg-gray-50 sm:grid-cols-9 dark:text-gray-400 dark:bg-gray-800">
          <div class="flex flex-wrap">
            <ValidationObserver tag="form" @submit.prevent="editWorkplace"  v-slot="{ invalid }" class="w-full">
              ...
              <div class="md:w-4/5 mx-auto p-2">
                ...
                <div v-if="workplace.workplace_images" class="relative w-72 h-[216px] bg-gray-800 rounded-lg">
                  ...
                  <div class="text-center w-72 h-[216px] flex justify-center">
                    <img :src="workplace.workplace_images[0].path" class="w-auto h-auto max-w-full max-h-full rounded-md"/>
                  </div>
                </div>
              ...
              <div class="md:w-4/5 mx-auto p-2">
                <button :disabled="invalid"
                  class="flex text-xs mx-auto text-white bg-blue-800 border-0 py-2 px-6 disabled:bg-gray-400  focus:outline-none hover:bg-blue-600 dark:text-gray-400 dark:bg-gray-700 dark:hover:bg-gray-600">登録</button>
              </div>
            </ValidationObserver>
          </div>
        </div>
...
export default {
  ...
  methods: {
    getWorkplace() {
      axios.get('/api/workplace/fetch_workplace', {
          params: {
            id: this.id
          }
        })
        ...
        .finally(() => {
          this.isLoading.workplace = false
        })
    },
    getClients() {
    ...
    editWorkplace() {
      ...
      this.workplace.workplace_images === null ? 
        formData.append('is_delete_registered_image', true) : formData.append('is_delete_registered_image', false)
      ...
      axios.post('/api/workplace/update', formData, config)
        .then(res => {
