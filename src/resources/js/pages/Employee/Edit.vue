      ...
      <div class="mx-4 my-5">
        <div class="w-full mb-2 rounded-lg shadow-xs dark:border-gray-700 bg-gray-50 sm:grid-cols-9 dark:text-gray-400 dark:bg-gray-800">
          <div class="flex flex-wrap">
            <ValidationObserver tag="form" @submit.prevent="editEmployee"  v-slot="{ invalid }" class="w-full">
              ...
              <div class="md:w-4/5 mx-auto p-2">
                ...
                <div class="py-2 mb-2">
                  <label class="border border-gray-500 text-gray-600 text-sm p-2 pr-32 cursor-pointer dark:text-gray-400">
                    <input type="file" @change="uploadProfileImage" class="hidden" accept="image/*">
                    <i class="fa-solid fa-plus mr-2"></i>追加
                  </label>
                </div>
                <div v-if="employee.employee_images && employee.employee_images.length > 0" class="relative w-72 h-[216px] bg-gray-800 rounded-lg">
                  ...
                  <div class="text-center w-72 h-[216px] flex justify-center">
                    <img :src="employee.employee_images[0].path" class="w-auto h-auto max-w-full max-h-full rounded-md"/>
                  </div>
                </div>
...
export default {
  ...
  methods: {
    getEmployee() {
      axios.get('/api/employee/fetch_employee', {
          params: {
            id: this.id
          }
        })
    ...
    editEmployee() {
      ...
      this.employee.employee_images === null ? 
      ...
      axios.post('/api/employee/update', formData, config)
        .then(res => {
