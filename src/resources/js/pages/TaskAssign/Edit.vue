      ...
      <div class="mx-4 mt-5">
        <div class="w-full mb-2 rounded-lg shadow-xs dark:border-gray-700 bg-gray-50 sm:grid-cols-9 dark:text-gray-400 dark:bg-gray-800">
          <div class="flex flex-wrap">
            <ValidationObserver tag="form" @submit.prevent="editTaskAssign"  v-slot="{ invalid }" class="w-full">
...
export default {
  ...
  methods: {
    async getTaskAssign() {
      await axios.get('/api/task_assign/fetch_task_assign', {
          params: {
            id: this.id
          }
        })
        ...
        .finally(() => {
          this.isLoading.taskAssign = false
        })
    },
    ...
    editTaskAssign() {
      axios.post('/api/task_assign/update', {
        id: this.taskAssign.id,
  ...
  async created() {
    await this.getTaskAssign()
    this.getClientsWithWorkplaces()
    this.getEmployees()
    this.getWorkplace(this.taskAssign.workplace_id)
  }
}
</script>